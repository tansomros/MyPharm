<%@ Page  Language="VB" AutoEventWireup="false" CodeBehind="Download.aspx.vb" Inherits="CPA.Download" %>
<%@ Import Namespace="System.Data" %>  
    ...
    <head runat="server">
        ...
        <title><%: Page.Title %> - ระบบฐานข้อมูลร้านยา</title>
    ...
    <table id="tbdata" class="table table-hover">
            ...
            <% For Each row As DataRow In dtDoc.Rows %>
                    <tr>              
                     ...
                     <a href="documents/<% =String.Concat(row("filepath")) %>" ><img src="images/icon/<% =String.Concat(row("fileicon")) %>" height="20"/></a>            
                    </td>
                  <td> <a href="documents/<% =String.Concat(row("filepath")) %>" target="_blank" ><% =String.Concat(row("Descriptions")) %></a></td>
                  <td class="text-center"> <a href="documents/<% =String.Concat(row("filepath")) %>" target="_blank" ><small> <% =String.Concat(row("Mwhen")) %></small></a></td>         
                </tr>
            <%  Next %>
                </tbody>               
              </table>  
